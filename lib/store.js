...
function friendlyAuthError(msg = "") {
  if (/already (been )?registered/i.test(msg)) return "Ese correo ya tiene un espacio. Entra con tu contraseña.";
  if (/invalid login credentials/i.test(msg)) return "Correo o contraseña incorrectos.";
  if (/password should be at least/i.test(msg)) return "La contraseña necesita al menos 6 caracteres.";
  if (/rate limit|too many/i.test(msg)) return "Muchos intentos seguidos — espera un minuto e inténtalo de nuevo.";
  if (/anonymous sign-ins are disabled/i.test(msg)) return "El registro está pausado un momento. Intenta de nuevo más tarde.";
...
export async function signUpSpace({ name, email, password }) {
  ...
  try {
    ...
    if (session?.user?.is_anonymous) {
      const { data, error } = await supabase.auth.updateUser({ email, password, data: { name } });
